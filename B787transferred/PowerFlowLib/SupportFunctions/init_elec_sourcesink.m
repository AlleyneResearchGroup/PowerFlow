function init_elec_sourcesink( mode )
%INIT_ELECTRICAL_SOURCESINK  Initialize source & sink blocks for thermal loads.
%   INIT_ELECTRICAL_SOURCESINK('MODE') initilization functions for electrical
...
if mode == 1    % SOURCE BLOCK INITIALIZATION
    %  searching for all electrical sink goto blocks
    [mdl,rem] = strtok(hblock.Path,'/');    % split model name into mdl
    if strcmp(hblock.ACDC_load,'AC') == 1
        goto_arr = find_system(mdl,'LookUnderMasks','all','FollowLinks','on','Name','elec_goto_ac');
    else
        goto_arr = find_system(mdl,'LookUnderMasks','all','FollowLinks','on','Name','elec_goto_dc');
    end
    ...
    if exist('fromarr') == 1
        ...
        if strcmp(hblock.ACDC_load,'AC') == 1
            try
                add_line(blk,'LoadBus/1','LoadSelect/1','autorouting','on');   
            catch
            end
        else
            try
    ...
    else    % warning statement if no electrical loads are found
        warning(['No electrical loads are connected to electrical source block: ',blk, ...
            '. No matching GOTO tags were found.  This output will be zero.'])
    end

    % clearing used variables from the workspace
    clear x y w h offset k fromarr loc mdl rem id hblock
...
elseif mode == 2        % SINK BLOCK INITIALIZATION
    ...
    while flg == 0
        ...
        % first searching for all electrical sink goto blocks
    ...
    set_param(blk,'MaskValues',[vals(1:3);num2str(randnum)]);

    % setting goto label 
    if strcmp(hblock.ACDC_load,'AC') == 1
        set_param([blk,'/elec_goto_ac'],'GotoTag',[hblock.load_loc,'_ElecAC_',num2str(hblock.unq_id)]);
    else
        set_param([blk,'/elec_goto_dc'],'GotoTag',[hblock.load_loc,'_ElecDC_',num2str(hblock.unq_id)]);
    end
