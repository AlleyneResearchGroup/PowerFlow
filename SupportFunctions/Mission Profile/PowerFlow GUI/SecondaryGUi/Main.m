...
% Last Modified by GUIDE v2.5 17-Apr-2014 16:28:34
...
for i = 1:N
    load(FileName{i})
    sim('Boeing737Sample_updated.mdl',MSSN.gen.time(end));
    save(['Sim' int2str(i)])
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Batfile] = uiputfile('Batch1.mat','Save Batch name');
N = str2double(get(handles.NumMSSN, 'string'));
FileName = cell(N,1);
B=1;
for i = 1:N
    A = uigetfile('*.mat','Select the MSSN file');
    if strcmp(num2str(A),'0') == 1
        B=0;
        break
    end
    FileName(i,:) = cellstr(A);
end
if B == 1
    save(Batfile,'FileName')
end
